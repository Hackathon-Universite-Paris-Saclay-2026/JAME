...
class InMemoryRunStore:
    def __init__(self) -> None:
        ...
        self._subscribers: dict[str, list[asyncio.Queue[ReasoningEvent]]] = defaultdict(list)
    ...
    def register_token(self, run_id: str, token: CancelToken) -> None:
        self._tokens[run_id] = token

    def cancel_run(self, run_id: str) -> bool:
        """Cancel a run immediately — fires the token so the running thread stops ASAP."""
    ...
    def unsubscribe(self, run_id: str, queue: asyncio.Queue[ReasoningEvent]) -> None:
