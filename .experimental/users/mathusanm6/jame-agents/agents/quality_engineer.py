...
def _maybe_compress(llm: ChatOpenAI, new_context: str, memory: str) -> str:
    full = (memory + "\n\n" + new_context).strip()
...
def _triage(llm: ChatOpenAI, specs: str, code_files: list[CodeFile]) -> dict[str, str]:
    ...
    resp = llm.invoke([HumanMessage(content=TRIAGE_PROMPT.format(
        specs=specs[:2000],
        file_list=file_list,
    ))])
...
def _analyse_file(
    ...
    resp = llm.invoke([HumanMessage(content=STATIC_ANALYSIS_PROMPT.format(
        priority=priority,
        specs=specs[:2000],
...
def _cross_file_check(
    ...
    resp = llm.invoke([HumanMessage(content=CROSS_FILE_PROMPT.format(
        specs=specs[:2000],
        per_file_summaries="\n".join(summaries),
    ))])
    thinking, raw = _strip_thinking(resp.content)
...
def quality_engineer_node(state: AgentState) -> dict:
    ...
    for code_file in ordered_files:
        ...
        result   = _analyse_file(llm, specs, code_file, priority)
    ...
    for result in per_file_results:
        ...
        memory = _maybe_compress(llm, f"Fix instructions for {code_file.path}.", memory)
