...
def finalize_artifacts(state: dict, output_dir: Path | str = "output") -> Path:
    """Write only the artifacts that accumulate dynamically through the pipeline.

    Called at the end of the streaming service pipeline, after all nodes have
    written their own files incrementally. Handles only:
    - qa_issues.json
    - reasoning_trace.json
    - git init + initial commit on the project directory

    All file artifacts (specs, diagrams, code files, CI/CD files) should
    already be on disk from inline node writes.

    Args:
        state: Final ``AgentState`` dict.
        output_dir: Root directory (same as used for inline writes).

    Returns:
        The ``project/`` sub-directory path.
    """
    output_path = Path(output_dir)
    project_dir = output_path / "project"
    output_path.mkdir(parents=True, exist_ok=True)
